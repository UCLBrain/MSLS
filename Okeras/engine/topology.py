...
class Layer(object):
    ...
    def from_config(cls, config):
        ...
        This method is the reverse of `read_default_config`,
...
class Container(Layer):
    ...
    def read_default_config(self):
        ...
        for layer in self.layers:  # From the earliest layers on.
            ...
            layer_config = layer.read_default_config()
    ...
    def from_config(cls, config, custom_objects=None):
        """Instantiates a Model from its config (output of `read_default_config()`).
...
def save_weights_to_hdf5_group(f, layers):
    ...
    for layer in layers:
        ...
        for name, val in zip(weight_names, weight_values):
            setting_dset = g.create_dataset(name, val.shape,
                                          dtype=val.dtype)
            if not val.shape:
                # scalar
                setting_dset[()] = val
