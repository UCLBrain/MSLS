...
from .segmentation_network_build_model import redefine_network_layers_for_training, fit_multifunctional_model, fit_thismodel
...
def testing_models(model, test_x_data, settings):
    ...
    for i in range(0, this_size):
        ...
        print(CSELECTED + "First model, input:", str(i), "probability map" + CEND)
        settings['test_name'] = 'First_model_class_' + str(int(i / 5) + 1) + '_probability_map.nii.gz'
    ...
    for i in range(0, this_size):
        # settings['test_name'] = 'label_' + labels[i] +'_prob_1.nii.gz'
        ...
        settings['test_name'] = 'Second_model_class_' + str(int(i / 5) + 1) + '_probability_map.nii.gz'
    ...
    for i in range(0, n_class):
             print(CSELECTED + "Final single class segmentation,", "class:", str(i + 1) + CEND)
             settings['test_name'] = 'CNN_GUI_final_single_class_' + str(i + 1) + '_segmentation.nii.gz'
             segmentation = segmentation_final_process(second_model_seg[i * 5],
                                                     settings,
                                                     save_nifti=True,
    ...
    print('')
    if this_size > 5:
        settings['test_name'] = 'CNN_GUI_final_multi_class_segmentation.nii.gz'
        output_scan_multi = np.zeros_like(second_model_seg[0])
        seg_im = {}

        for i in range(0, n_class):
            file_n = 'CNN_GUI_final_single_class_' + str(i + 1) + '_segmentation.nii.gz'
            image = nib.load(os.path.join(settings['test_folder'], settings['run_testing'], settings['model_name'], file_n))
...
def run_testing(model,
    ...
    # if check_tolerance_error(seg_image, settings, flair_image.header.get_zooms()):
    #       if settings['debug']:
    #          print("> DEBUG ", scans[0], "lesion volume below ", settings['min_error'], 'ml')
    #       seg_image = np.zeros_like(flair_image.get_data().astype('float32'))
    #
    if index == (int(index / 5) * 5):
