...
from Okeras.layers import Dense, Dropout, Flatten, Input
from Okeras.layers.convolutional import Conv3D, MaxPooling3D
from Okeras.layers.advanced_activations import PReLU as prelu
from Okeras.layers.normalization import BatchNormalization as BN
from Okeras import backend as K
from Mkeras import regularizers
from Okeras.models import Model
import Okeras
# K.set_image_dim_ordering('th')
K.set_image_data_format('channels_first')

def build_annotation_network(settings, model_name=None):
    ...
    if this_size > 5:
        merged = Okeras.layers.Concatenate(axis=1)(net_IN)
    # net_input = Input(name='in1', shape=(channels,) + settings['patch_size'])
        ...
        merged = Okeras.layers.Concatenate(axis=1)([net_input1,net_input2,net_input3,net_input4,net_input5])
    ...
    if this_size > 5:
         ...
         net_output1 = Dense(units=2, name='output1', activation='softmax')(layer)
         net_output2 = Dense(units=2, name='output2', activation='softmax')(layer)
         net_output3 = Dense(units=2, name='output3', activation='softmax')(layer)
         net_output4 = Dense(units=2, name='output4', activation='softmax')(layer)
         net_output5 = Dense(units=2, name='output5', activation='softmax')(layer)
         model = Model(inputs=[net_input1, net_input2, net_input3, net_input4, net_input5],
                       outputs=[net_output1, net_output2, net_output3, net_output4, net_output5])
