...
import threading
import Mkeras
from Mkeras.callbacks import EarlyStopping, ModelCheckpoint, TensorBoard
from .segmentation_network_build_nets import build_network
from Mkeras import optimizers, losses
...
Mkeras.backend.set_image_data_format('channels_first')
...
def build_and_compile_models(settings):
    ...
    Output:
    - nets = list of NeuralNets (CNN_GUI1, CNN_GUI2)
    """
...
def redefine_network_layers_for_training(model, settings, num_layers=1, number_of_samples=None):
    ...
    if this_size > 5:
        for i in range(0, int(this_size / 5)):
         ...
         net.get_layer('approximated_5to1_output').trainable = True
...
def fit_multifunctional_model(model, x_train, y_train, settings, X_val, Y_val, initial_epoch=0):
    ...
    if this_size == 5:
        for label in range(0, 5):
            y_train[label] = Mkeras.utils.to_categorical(y_train[label] == 1,
                                                         len(np.unique(y_train[label] == 1)))
            Y_val[label] = Mkeras.utils.to_categorical(Y_val[label] == 1,
