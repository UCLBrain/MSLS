...
class Model(Container):
    ...
    def compile(self, optimizer, loss, option=None, metrics=None, loss_weights=None,
        ...
        print('\x1b[6;30;41m' + 'Warning! this code uses modified Keras compiler' + '\x1b[0m')
        print('\x1b[6;30;41m' + '                                               ' + '\x1b[0m')

        print('')
        print('\x1b[6;30;43m' + 'Number of classes:' + '\x1b[0m',  option)
        ...
        if masks is None:
            ...
            # masks = [None for _ in range(0, 5 * option)]
        ...
        for j in range(0, option):
            for i in range(0, 5):
                ...
                #     self.targets.append(None)
                # else:
                shape = self.internal_output_shapes[j]
                # name = self.output_names[0]
                name = ["1", "2", "3", "4", "5"]
                target = K.placeholder(ndim=len(shape),
                                       name=name[i] + str(j) + '_target',
                                       sparse=K.is_sparse(self.outputs[j]),
                                       dtype=K.dtype(self.outputs[j]))
                self.targets.append(target)
                self._feed_targets.append(target)
        ...
        for j in range(0, option):
            for i in range(0, 5):
                # if i in skip_indices:
                #     continue
                y_true = self.targets[i + 5 * j]
                # || y_pred, y_true1|| + || y_pred, y_true2|| + || y_pred, ...|| + ...
                y_pred = self.outputs[j]
                weighted_loss = weighted_losses[j]
                sample_weight = sample_weights[j]
                mask = masks[j]
                loss_weight = loss_weights_list[j]
                output_loss = weighted_loss(y_true, y_pred,
                                            sample_weight, mask)
                ...
                if len(self.outputs) > 1:
                    self.metrics_tensors.append(output_loss)
                    self.metrics_names.append(self.output_names[j] + '_loss')
                if total_loss is None:
                    total_loss = loss_weight * output_loss
                else:
                    total_loss += loss_weight * output_loss
        ...
        for j in range(0, option):
            for i in range(0, 5):
                ...
                y_true = self.targets[i + 5 * j]
                y_pred = self.outputs[j]
                output_metrics = nested_metrics[j]
                for metric in output_metrics:
                    if metric == 'accuracy' or metric == 'acc':
                        # custom handling of accuracy
                        # (because of class mode duality)
                        output_shape = self.internal_output_shapes[j]
                        acc_fn = None
                        if output_shape[-1] == 1 or self.loss_functions[j] == losses.binary_crossentropy:
                            # case: binary accuracy
                            acc_fn = metrics_module.binary_accuracy
                        elif self.loss_functions[j] == losses.sparse_categorical_crossentropy:
                            # case: categorical accuracy with sparse targets
                            acc_fn = metrics_module.sparse_categorical_accuracy
                        else:
                            acc_fn = metrics_module.categorical_accuracy
                        ...
                        masked_fn = _masked_objective(acc_fn)
                        append_metric(j, 'Keras_accuracy', masked_fn(y_true, y_pred, mask=masks[j]))
                    else:
                        metric_fn = metrics_module.get(metric)
                        masked_metric_fn = _masked_objective(metric_fn)
                        metric_result = masked_metric_fn(y_true, y_pred, mask=masks[j])
                        metric_result = {
                            metric_fn.__name__: metric_result
                        }
                        for name, tensor in six.iteritems(metric_result):
                            append_metric(j, name, tensor)
        ############## here #####################
    ...
    def train_on_batch(self, x, y,
        ...
        # here begin...........
        # for i in range(0, 5):
        #     x, y_tmp[i], sample_weights = self._standardize_user_data(
        #     x, y[i],
        #     sample_weight=sample_weight,
        #     class_weight=class_weight,
        #     check_batch_axis=True)
