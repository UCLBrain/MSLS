...
def save_model(model, filepath, overwrite=True):
    ...
    def get_json_type(obj):
        ...
        # e.g. optimizer, layer
        if hasattr(obj, 'read_default_config'):
            return {'class_name': obj.__class__.__name__,
                    'config': obj.read_default_config()}
    ...
    f.attrs['model_config'] = json.dumps({
        ...
        'config': model.read_default_config()
        ...
        if isinstance(model.optimizer, optimizers.TFOptimizer):
            ...
            f.attrs['training_config'] = json.dumps({
                'optimizer_config': {
                    ...
                    'config': model.optimizer.read_default_config()
                },
                'loss': model.loss,
            ...
            if symbolic_weights:
                ...
                for name, val in zip(weight_names, weight_values):
                    setting_dset = optimizer_weights_group.create_dataset(
                        name,
                        val.shape,
                        dtype=val.dtype)
                    if not val.shape:
...
class Sequential(Model):
    ...
    def read_default_config(self):
        if isinstance(self.layers[0], legacy_layers.Merge):
            return self.legacy_read_default_config()

        config = []
        for layer in self.layers:
            config.append({'class_name': layer.__class__.__name__,
                           'config': layer.read_default_config()})
    ...
    def legacy_read_default_config(self):
        ...
        if isinstance(self.layers[0], legacy_layers.Merge):
            ...
            merge_config = self.layers[0].read_default_config()
            ...
            config.append({'class_name': self.layers[0].__class__.__name__,
                           'config': self.layers[0].read_default_config()})
        for layer in self.layers[1:]:
            config.append({'class_name': layer.__class__.__name__,
                           'config': layer.read_default_config()})
        return copy.deepcopy(config)
